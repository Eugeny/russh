    ...
    AffinePoint<C>: FromEncodedPoint<C> + ToEncodedPoint<C>,
{
    ...
    fn server_dh(&mut self, exchange: &mut Exchange, payload: &[u8]) -> Result<(), crate::Error> {
        ...
        let server_secret = elliptic_curve::ecdh::EphemeralSecret::<C>::random(&mut rand::rng());
    ...
    fn client_dh(
        ...
        let client_secret = elliptic_curve::ecdh::EphemeralSecret::<C>::random(&mut rand::rng());
...
mod tests {
    ...
    fn test_shared_secret() {
        let mut party1 = EcdhNistPKex::<NistP256, Sha256> {
            local_secret: Some(EphemeralSecret::<NistP256>::random(&mut rand::rng())),
            shared_secret: None,
            _digest: PhantomData,
        };
