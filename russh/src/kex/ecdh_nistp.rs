    ...
    AffinePoint<C>: FromSec1Point<C> + ToSec1Point<C>,
{
    ...
    fn server_dh(&mut self, exchange: &mut Exchange, payload: &[u8]) -> Result<(), crate::Error> {
        ...
        let server_secret = EphemeralSecret::<C>::generate_from_rng(&mut rng());
    ...
    fn client_dh(
        ...
        let client_secret = EphemeralSecret::<C>::generate_from_rng(&mut rng());
...
mod tests {
    ...
    fn test_shared_secret() {
        let mut party1 = EcdhNistPKex::<NistP256, Sha256> {
            local_secret: Some(EphemeralSecret::<NistP256>::generate_from_rng(&mut rng())),
            shared_secret: None,
            _digest: PhantomData,
        };
