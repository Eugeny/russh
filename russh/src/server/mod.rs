...
//! #[derive(Clone)]
//! struct Server {
//!     channels: Arc<Mutex<HashMap<(usize, ChannelId), Channel<Msg>>>>,
//!     id: usize,
//! }
//!
...
//!             .lock()
//!             .await
//!             .insert((self.id, channel.id()), channel);
//!
//!         Ok(true)
//!     }
...
//!         for ((_id, _channel_id), ref mut channel) in self.channels.lock().await.iter_mut() {
