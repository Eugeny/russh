...
mod test_compress {
    ...
    async fn compress_local_test() {
        ...
        let authenticated = session
            .authenticate_publickey(whoami::username(), Arc::new(client_key))
            .await
            .unwrap();
...
mod test_server_channelstream {
    ...
    #[tokio::test]
    async fn test_server_channelstream() {
        let _ = env_logger::try_init();
        ...
        let join = tokio::spawn(async move {
            ...
            let mut server = sh.new_client(socket.peer_addr().ok());
            let auth_waiter = server.get_auth_waiter();
            let channel_waiter = server.get_channel_waiter();
            server::run_stream(config, socket, server).await.unwrap();

            auth_waiter.await.unwrap();

            let channel = channel_waiter.await.unwrap();
        ...
        stream.write(&b"hey there!"[..]).await.unwrap();
    ...
    struct ServerHandle {
        did_auth: Option<tokio::sync::oneshot::Sender<()>>,
        channel: Option<tokio::sync::oneshot::Sender<Channel<Msg>>>,
    }
    ...
    impl ServerHandle {
        ...
        fn get_channel_waiter(&mut self) -> tokio::sync::oneshot::Receiver<Channel<Msg>> {
            let (tx, rx) = tokio::sync::oneshot::channel::<Channel<Msg>>();
